<?php
    //
    //  $Id
    //  Revision 1.4.1  2006/08/29 		  AK
    //  Just eliminated some php notices ...
    //
    //  $Log: week.php,v $
    //  Revision 1.4  2003/02/20 13:50:18  wk
    //  - bugfix, i overwrote the $data :-(
    //
    ...
    $saved = $pageHandler->save(@$_REQUEST['newData']);  // AK : if newData not in REQUEST, save returns an empty object
    ...
    if (sizeof($_times)) {
        ...
        foreach ($_times as $key=>$aTime) {
            if ($dayName!=date('l',$aTime['timestamp'])) {
                ...
                if (isset($times[$cnt]) && !isset($times[$cnt][sizeof($times[$cnt])-1]['duration'])) {  //AK 2nd isset to avoid notice
                    $times[$cnt][sizeof($times[$cnt])-1]['_endOfDay'] = true;
                }
            
                $midnite = mktime(0,0,0,date('m',$aTime['timestamp']),date('d',$aTime['timestamp']),date('Y',$aTime['timestamp']));
                $_data['id'] = 0;
            ...
            $_title.= isset($aTime['duration']) ? " ({$aTime['duration']}) " : ' ';  //AK : isset to avoid notice
            $_title.= $projectTreeDyn->getPathAsString($aTime['projectTree_id']);
            $_title.= " - {$aTime['_task_name']}";
            $aTime['_title'] = $_title;
            $times[$cnt][] = $aTime;
            
            $colorLegend[strtolower($aTime['_task_name'])] = array('color'=>$aTime['_task_color'],'name'=>$aTime['_task_name']);
        }
