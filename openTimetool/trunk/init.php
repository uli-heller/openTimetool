...
//    $config->setFeature('translate','develop');
    ...
    */
    $options = array('columns'=>array(  'session_id'    =>  'session_id()'
                                        ,'user_id'      =>  '($GLOBALS["userAuth"] && !Auth::isError($GLOBALS["userAuth"]) && $GLOBALS["userAuth"]->isLoggedIn())?'.
                                                            '$GLOBALS["userAuth"]->getData("id"):0'
                                    )
                    );
    ...
    */ 
    ...
    if ($account->isAspVersion()) {
        ...
        if (isset($session->account->ttVersion) && $session->account->ttVersion!=$config->applVersion) {
            $_oldVersion = $config->applVersion;
            $config->applPathPrefix = str_replace($config->applVersion,$session->account->ttVersion,$config->applPathPrefix);
            $config->applVersion = $session->account->ttVersion;
            // redirect to the new url and dont send the session id with it!
            HTTP_Header::redirect(str_replace("/$_oldVersion/","/{$config->applVersion}/",$_SERVER['PHP_SELF']),false);
        }
    ...
    */
    ...
    {
        if( DB::isError($db = DB::connect($config->dbDSN,array('debug' => true,'persistent'=>true))) )
        {
    ...
    */
    if( !$config->isLiveMode() ) $processingTimer->setMarker('init.php before tpl-instanciating');
    ...
    if( !$config->isLiveMode() )    $processingTimer->setMarker('init.php after tpl-instanciating');

	// AK We can set here various debugging switches when running in develop-mode (->config.php)
    if( $config->runMode == 'develop' )    // only on my machine i want this, the other shall see cached files
    {
    ...
    $tpl->registerPrefilter(array(&$modifiers,'imgSrc'),array($config->mediaRoot,$config->vMediaRoot));

    require_once('I18N/Messages/Translate.php');
    require_once('HTML/Template/Xipe/Filter/Translate.php');

    if( !$config->isLiveMode() ) $processingTimer->setMarker('init.php tpl before detecting lang');

    $translateOptions = array(  'tablePrefix'=>TABLE_TRANSLATE_,'sourceLanguage'=>'en',
    ...
    if( !$config->isLiveMode() ) $processingTimer->setMarker('init.php tpl before lang-stuff');
...
*/
    ...
    if( !$config->isLiveMode() ) $processingTimer->setMarker('init.php tpl block done');
...
/*
    ...
    if ($headers['Authorization']) {
        ...
        $this->login($authData[0],$authData[1]); // log the user in, using the received data
    }
*/
    /**
    *        user authentication
...
//    $userAuth = Auth::setup( 'NIS' , 'vpnis' , $options );
    ...
    {                           
        ...
        if( $userAuth->isLoggedIn() && !$userAuth->getData('id') )
        {
            ...
            // since we might authenticate against another source, such as imap where we dont get those data from!
            $user->setWhere('login='.$db->quote($userAuth->getUsername()));
            $users = $user->getAll();
            $user->setWhere();                          // reset the where clause, so the user can be used normally again
            if( sizeof($users) == 1 )
            {
                $userAuth->setData( $users[0] );
                // force to get all the account-data!, since they might have changed, this has no effect on a NET version
...
*/
    ...
    */
    $applError->setOption( 'table' , TABLE_ERRORLOG );  // the TABLE_ERRORLOG was not defined when we instanciated the object
//FIXXXXXME this is not very nice, check someway else if a db is selected
    ...
    // this could also be done by overriding the concerning methods ... i dont know what is better
    // i pass options now :-)
    $options = array(   'layoutPath'        =>  $config->applRoot.'/layout',
                        'virtualLayoutPath' =>  $config->applPathPrefix.'/layout',
                        'templateDir'       =>  $tpl->options['templateDir'],   // $config->applRoot would be the same, but this is 'more' secure, since we need the tempalte dir :-)
                        'virtualTemplateDir'=>  $config->applPathPrefix );
    $layout = new vp_Page_Layout($options);
    // be sure to have set 'main' as default, if someone wnats something else it needs to be done explicitly
