...
//print "set ".$val['name']."  $key = $newVal<br>";
            ...
            if (!empty($val['children'])) {
            	// AK : due to php5 we have to copy $val['children'] in a local variable 
            	// and pass the reference to this variable to array_walk. Simple but you'll get
            	// otherweise : Fatal Error : Only variables can be passed by reference ...
            	$tmp_val = $val['children'];
//	print_r($tmp_val);echo "<br>";
//	echo "val = ".gettype($val['children'])."<br>";
            	if(is_array($tmp_val)) { 
            		array_walk($tmp_val,array(&$this,'_update'),$data);
            		// AK : stuff the modified array back (needed due to use of local variable)
            		$val['children'] = $tmp_val;  
            	}
            }
        }
    }
